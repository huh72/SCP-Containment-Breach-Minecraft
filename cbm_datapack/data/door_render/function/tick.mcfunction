###LCZ
execute as @e[type=item_display,tag=aj.door0.root,tag=!render_constantly] at @s if entity @n[tag=door_marker,tag=!dr.not_rendered,tag=!card] unless entity @a[distance=..20] run function animated_java:door0/remove/this
execute as @e[type=marker,tag=door_marker,tag=dr.not_rendered,tag=!card,tag=!render_constantly] at @s if entity @a[distance=..19] run function doors:respawn_door
