...
#&7stats
scoreboard players operation @s acc = acc stat_usps
...
execute store result storage gun_main shot_cd int 1 run scoreboard players get shot_cd stat_usps
...
#reload start 
execute as @s[scores={ammo=0},tag=!reloading] run function guns:weapons/usps/reload/start
#reload end
execute as @s[scores={reload=0},tag=reloading] run function guns:weapons/usps/reload/end