...
execute if entity @a[tag=confirmed_target,tag=!dead,distance=..16] if score @s[tag=!notHostile] shot_cd matches 0 as @n[tag=aj.mtf.root] at @s run function guns:shoot_helper with storage cb:mtf