...
execute as @e[type=marker,tag=elevator,tag=new] at @s run tellraw @a [{"text":"[ ! ]","color":"white"},{"text":" A new elevator detected! Don't forget to initialize it's tags and variables THROUGH vscode!","color":"yellow"}]
