#g vars : spawn
execute if score .timerCanBeIncreased 106spawnTimer matches 1 if score . 106spawnTimer < .request 106spawnTimer if score .106 isContained matches 0 unless entity @a[tag=location.939storage] unless entity @a[tag=location.049chamber] unless entity @a[tag=location.106chamber] unless entity @a[tag=location.surfaceA] unless entity @a[tag=location.surfaceB] unless entity @a[tag=location.pocketDimension] unless score .speech event.079 matches 0..1099 run scoreboard players add . 106spawnTimer 1
