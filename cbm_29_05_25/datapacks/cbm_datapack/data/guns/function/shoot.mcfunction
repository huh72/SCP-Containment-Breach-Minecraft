...
#recoil
execute store result storage recoil xa int 1 run random value -2..2 cb:acc
execute store result storage recoil ya int 1 run random value 1..5 cb:acc
#temp
...
$execute as @s at @s anchored eyes positioned ^ ^ ^1 rotated ~$(xb).$(xa) ~$(yb).$(ya) run function guns:bullet/spawn with storage minecraft:gun_main
