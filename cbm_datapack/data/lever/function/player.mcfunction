...
execute if score @s right_click_item matches 1.. as @n[tag=lever] unless entity @s[tag=uninteractibale] if entity @s[tag=storage939, tag=up, tag=!proceed] as @n[type=marker,tag=door_marker_hcz,tag=storage939door] at @s if entity @s[tag=closed] run function doors:open_storage_939
