playsound cb:pd.exit ambient @s ~ ~ ~ 1 1 1
stopsound @s * cb:pd.ambient
execute at @e[type=marker, tag=door_marker, tag=cantBeEscapedAt, limit=1, sort=random] run tp @s[tag=caughtInLcz] ~ ~ ~0.35
execute at @e[type=marker, tag=door_marker_hcz, tag=cantBeEscapedAt, limit=1, sort=random] run tp @s[tag=caughtInHcz] ~ ~ ~0.35
execute at @e[type=marker, tag=door_marker_ez, tag=cantBeEscapedAt, limit=1, sort=random] run tp @s[tag=caughtInEz] ~ ~ ~0.35

tag @s[tag=caughtInLcz] add lcz
tag @s[tag=caughtInHcz] add hcz
tag @s[tag=caughtInEz] add ez

tag @s remove hitby106

scoreboard players set @s[tag=lcz] zoneambientlcz 1
scoreboard players set @s[tag=hcz] zoneambienthcz 1
scoreboard players set @s[tag=ez] zoneambientez 1

execute if entity @s[tag=hcz] run function shader_control:effects/yellow/set {"value":"0.2"}
execute if entity @s[tag=ez] run function shader_control:effects/blue/set {"value":"0.2"}

execute if entity @s[tag=!caughtInLcz, tag=!caughtInHcz, tag=!caughtInEz] at @e[type=marker, tag=door_marker, limit=1, sort=random] run tp @s ~ ~ ~0.35

tag @s remove location.pocketDimension

function shader_control:effects/green/set {"value":"0.0"}