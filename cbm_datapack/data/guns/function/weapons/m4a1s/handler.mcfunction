...
scoreboard players operation @s damage = damage stat_m4a1
...
execute store result storage gun_main damage int 1 run scoreboard players get damage stat_m4a1s
...
#reload start 
execute as @s[scores={ammo=0},tag=!reloading] run function guns:weapons/m4a1s/reload/start
#reload end
execute as @s[scores={reload=0},tag=reloading] run function guns:weapons/m4a1s/reload/end