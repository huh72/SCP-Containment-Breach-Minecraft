...
#stop/go conditions:
execute at @n[tag=ulgrin,tag=!stop] unless entity @n[tag=ulgrinP,distance=..8] run function int:path/stop
execute at @n[tag=ulgrin,tag=stop] if entity @n[tag=ulgrinP,distance=..8] run function int:path/proceed

#termination
execute as @a[tag=!dead] at @s unless entity @n[tag=ulgrin,distance=..8] unless entity @n[tag=ulgrinP,distance=..8] run function int:terminate
